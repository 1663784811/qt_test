...
VdSwitch::VdSwitch(QWidget *parent) : QWidget{parent}
{
    checked = false;
    ...
    timer->setInterval(5);
    connect(timer, &QTimer::timeout, this, &VdSwitch::updateValue);
    setFont(QFont("Microsoft Yahei", 10));
    this->setCursor(QCursor(Qt::PointingHandCursor));
}

void VdSwitch::mousePressEvent(QMouseEvent *)
{
    checked = !checked;
...
void VdSwitch::resizeEvent(QResizeEvent *)
{
    // 每次移动的步长为宽度的 40分之一
    step = width() / 40;
    // 尺寸大小改变后自动设置起点坐标为终点
...
void VdSwitch::paintEvent(QPaintEvent *)
{
    // 绘制准备工作,启用反锯齿
...
void VdSwitch::drawBg(QPainter *painter)
{
    painter->save();
...
void VdSwitch::drawSlider(QPainter *painter)
{
    painter->save();
    painter->setPen(Qt::NoPen);
    if (!checked) {
...
void VdSwitch::drawText(QPainter *painter)
{
    painter->save();
    if (!checked) {
...
void VdSwitch::drawImage(QPainter *painter)
{
    painter->save();
    QPixmap pix;
    if (!checked) {
...
void VdSwitch::setChecked(bool checked)
{
...
void VdSwitch::setButtonStyle(VdSwitch::ButtonStyle buttonStyle)
{
    this->buttonStyle = buttonStyle;
    update();
}
void VdSwitch::setBgColor(QColor bgColorOff, QColor bgColorOn)
{
    this->bgColorOff = bgColorOff;
    this->bgColorOn = bgColorOn;
    update();
}
void VdSwitch::setSliderColor(QColor sliderColorOff, QColor sliderColorOn)
{
    this->sliderColorOff = sliderColorOff;
    this->sliderColorOn = sliderColorOn;
    update();
}
void VdSwitch::setTextColor(QColor textColorOff, QColor textColorOn)
{
    this->textColorOff = textColorOff;
    this->textColorOn = textColorOn;
    update();
}
void VdSwitch::setText(QString textOff, QString textOn)
{
    this->textOff = textOff;
    this->textOn = textOn;
    update();
}
void VdSwitch::setImage(QString imageOff, QString imageOn)
{
    this->imageOff = imageOff;
    this->imageOn = imageOn;
    update();
}
void VdSwitch::setSpace(int space)
{
    this->space = space;
    update();
}
void VdSwitch::setRectRadius(int rectRadius)
{
